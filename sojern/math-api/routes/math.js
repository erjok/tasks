...
const createNumbersValidationChain = () =>
    param('numbers')
        ...
        .custom(value => {
            if (value.some(Number.isNaN)) throw new Error('Numbers must be a comma-separated list of numbers.');
            if (value.length === 0) throw new Error('Numbers must have at least one number.');
...
router.get('/avg/:numbers', createNumbersValidationChain(), reqValidator, (req, res) => {
    const avg = calculator.avg(req.params.numbers);
    res.status(200).json({ avg });
});

router.get('/median/:numbers', createNumbersValidationChain(), reqValidator, (req, res) => {
